...
CS_PLUGIN_NAMESPACE_BEGIN (SoftAnim2)
{
  ...
  void SoftBodyControl::SetSoftBody (CS::Physics::iSoftBody* body,
				     CS::Physics::MeshDuplicationMode duplicationMode)
  {
    ...
    size_t vertexCount = duplicationMode != CS::Physics::MESH_DUPLICATION_NONE ?
  ...
  void SoftBodyControl::Update (csTicks current, int num_verts, uint32 version_id)
  {
    ...
    for (size_t i = 0; i < softBody->GetVertexCount (); i++)
    {
      ...
      int meshIndex = duplicationMode == CS::Physics::MESH_DUPLICATION_INTERLEAVED ? 2 * i : i;
	...
	normals[meshIndex] = softBody->GetVertexNormal (i);
      ...
      if (duplicationMode != CS::Physics::MESH_DUPLICATION_NONE)
      {
	size_t index = duplicationMode == CS::Physics::MESH_DUPLICATION_INTERLEAVED ?
						   ...
						   int num_normals, uint32 version_id)
  {
    ...
    CS_ASSERT (duplicationMode != CS::Physics::MESH_DUPLICATION_NONE ?
