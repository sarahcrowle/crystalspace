...
#include "networkeventservertest.h"
...
NetworkEventServerTest::NetworkEventServerTest ()
{
  SetApplicationName ("CrystalSpace.NetworkEventServerTest");
}
...
bool NetworkEventServerTest::HandleEvent (iEvent &ev)
{
...
bool NetworkEventServerTest::OnKeyboard(iEvent& ev)
{
...
void NetworkEventServerTest::Frame ()
{
...
bool NetworkEventServerTest::OnInitialize(int /*argc*/, char* /*argv*/ [])
{
...
bool NetworkEventServerTest::SetupModules ()
{
  ...
  socketManager = csQueryRegistry<iSocketManager>(GetObjectRegistry ());
  ...
  eventManager = csQueryRegistry<iEventManager>(GetObjectRegistry ());
  if(!eventManager.IsValid())
  {
    csPrintf("failed to query event manager\n");
    return -1;
  }
  
  csRef<iAddress> address = socketManager->Resolve(nullptr, "12345", CS_SOCKET_FAMILY_IP4, CS_SOCKET_PROTOCOL_TCP);
  if(!address.IsValid())
  {
...
void NetworkEventServerTest::OnExit()
{
}

bool NetworkEventServerTest::Application()
{
...
int main (int argc, char* argv[])
{
   ...
   */
  return csApplicationRunner<NetworkEventServerTest>::Run (argc, argv);
}
