...
def GetUniqueFileName(obj):
    ...
    if fileName not in UNIQUE_FILENAMES:
        ...
        UNIQUE_FILENAMES[fileName][path] = str(
            len(UNIQUE_FILENAMES[fileName])) + '-'
...
def SaveImage(self, path):
    ...
    try:
        if self.packed_file:
            ...
            library = os.path.split(bpy.path.abspath(self.library.filepath))[
                0] if self.library else None
