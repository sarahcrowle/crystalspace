...
class RenderBuffers:
    ...
    def __init__(self, **kwargs):
        ...
        if 'mappingBuffers' in kwargs:
            ...
            print(
                "ERROR: mapping buffers are not defined to build the render buffers")
    ...
    def GenerateBuffers(self):
        ...
        for i, ob in enumerate(self.meshData):
            ...
            for indexCS, indexBlender in enumerate(self.mappingVertices[i]):
                ...
                self.positionBuffer.AddElement(
                    tuple(map(lambda j: scale[j] * pos[j], range(3))))
                if doubleSided:
                    # Duplicate the vertex coordinates in case of a double
                    # sided mesh
                    self.positionBuffer.AddElement(
                        tuple(map(lambda j: scale[j] * pos[j], range(3))))
                ...
                # Flip Y and Z axis.
                self.normalBuffer.AddElement(
                    tuple(self.mappingNormals[i][indexCS]))
                if doubleSided:
                    # Duplicate and inverse the vertex normal in case of a
                    # double sided mesh
                ...
                face, vi = findFace(
                    self.mappingBuffers[i][indexBlender], indexCS)

                # Add uv coordinates of this vertex to the CS texture
                # coordinates buffer
                if tface and face != -1:
                    uv = tface[face].uv[vi]
                    uvco = (uv[0], 1.0 - uv[1])
                ...
                if doubleSided:
                    # Duplicate the uv coordinates in case of a double sided
                    # mesh
                    self.texCoordBuffer.AddElement(uvco)

                # Add the vertex color to the CS color buffer
                if cface and face != -1:
                    colors = [cface[face].color1, cface[
                        face].color2, cface[face].color3, cface[face].color4]
                    col = tuple(colors[vi])
                else:
                    col = (1.0, 1.0, 1.0)
