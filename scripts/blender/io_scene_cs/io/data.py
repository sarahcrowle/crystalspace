                ...
                for i in tt:
                    ...
                    for mapV in mappingBuffer[face.vertices[i]]:
                        if mapV['face'] == index and mapV['vertex'] == i:
                            ...
                            indexGroups[triplet].append(
                                firstIndex + mapV['csVertex'])
                            indexV += 1
                            break
            else:
                # Double sided mesh
                ...
                for i in tt:
                    for mapV in mappingBuffer[face.vertices[i]]:
                        if mapV['face'] == index and mapV['vertex'] == i:
                            ...
                            frontFaceIndices.append(
                                firstIndex + 2 * mapV['csVertex'])
                            break
...
def GetSubMeshes(self, name='', mappingBuffer=None, numVertices=0):
    ...
    indexV = numVertices
    indexGroups = {}
    indexV, indexGroups = self.GetSubMeshesRaw(
        name, indexV, indexGroups, mappingBuffer)
...
def GetCSMappingBuffers(self):
    ...
    # Determine the UV texture: we only use the active UV texture or the first
    # one
    ...
    for indexF, face in enumerate(self.all_faces):
        ...
        for i in tt:
            ...
            smooth = forceSmooth or (
                face.use_smooth and not self.use_auto_smooth)
            if not smooth and not faceNormal:
                print("WARNING: mesh '%s' has no normals. Setting smooth mode." %
                      (self.name))
                forceSmooth = True
                smooth = True
            if not smooth and face.use_smooth and self.use_auto_smooth:
                for indexf, f in enumerate(self.all_faces):
                    if face != f and f.use_smooth and len(faceVerts.intersection(set(f.vertices))) != 0:
                        ...
                        # auto-smooth angle have 'smooth' normals, i.e. use
                        # vertex normals
            ...
            if len(mapBuf[face.vertices[i]]) == 0:
                ...
                mappingVertex = {
                    'face': indexF, 'vertex': i, 'csVertex': csIndex}
                    ...
                    for mappedI, mappedV in enumerate(mapVert):
                        if mappedV == face.vertices[i]:
                            ...
                            if smooth:
                                ...
                                mappingVertex = {
                                    'face': indexF, 'vertex': i, 'csVertex': mappedI}
                                mapBuf[face.vertices[i]].append(mappingVertex)
                            else:
                                # Not a smooth mesh
                                vertexFound = False
                                for mappedI, mappedV in enumerate(mapBuf[face.vertices[i]]):
                                    if faceNormal == self.all_faces[mappedV['face']].normal:
                                        ...
                                        vertexFound = True
                                        mappingVertex = {
                                            'face': indexF, 'vertex': i, 'csVertex': mappedV['csVertex']}
                                        mapBuf[face.vertices[i]].append(
                                            mappingVertex)
                                        break
                                if not vertexFound:
...
def GetBoneNames(bones):
    ...
    def GetChildBoneNames(children, boneNames, index):
        for child in children:
            ...
            boneNames, index = GetChildBoneNames(
                child.children, boneNames, index)
...
def AsCSSkeletonAnimations(self, func, depth, armatureObject, dontClose=False):
    ...
    for action in actions:
        ...
        for curve in action.fcurves:
            if not curve.is_valid:
                print("Error: Curve is not valid",
        ...
        if len(lostBones):
            print("WARNING: Action '%s' not exported: the following bones are not part of the armature" %
                  (action.name), lostBones)
            continue

        if len(boneKeyframes) == 0:
            print("WARNING: Action '%s' not exported: no keyframe defined" %
                  (action.name))
            continue
    ...
    """
        ...
        if len(exportedActions) == 1:
            func(' ' * depth + '<animationpacket name="%s_packet">' %
                 (armatureName))
    ...
    if len(exportedActions) != 0:
        ...
        for action in exportedActions:
            ...
            func(' ' * depth + '      <node type="animation" animation="%s"/>' %
                 (action.name))
            func(' ' * depth + '    </state>')
        func(' ' * depth + '  </node>')
        func(' ' * depth + '</animationpacket>')
        print('INFO: %s animation(s) exported for skeleton "%s_rig"' %
              (len(exportedActions), armatureName))
    ...
    if len(exportedActions):
        func(' ' * depth + '  <animationpacket>%s_packet</animationpacket>' %
             (armatureName))
    func(' ' * depth + '</skeleton>')
    print('INFO: %s bone(s) exported for skeleton "%s_rig"' %
          (len(boneNames), armatureName))
...
def GetBoneInfluences(self, **kwargs):
        ...
        for indexV in mappingVertices[index]:
            ...
            for groupEl in ob.data.vertices[indexV].groups:
                ...
                if groupEl.weight > epsilon:
                    ...
                    groupIndex = find(
                        lambda gr: gr.index == groupEl.group, ob.vertex_groups)
                    if groupIndex == None:
    ...
    if len(lostGroups):
        print(
            "WARNING: vertex groups defined on unknown bones; corresponding bone influences have been lost!", lostGroups)
    if lostInfluences:
...
def AsCSSocket(self, func, depth, socketObject, armature):
    ...
    """
    print("Export socket: object '%s' attached to bone '%s'" %
          (socketObject.name, self.name))
    ...
    func(' ' * depth + '<socket name="%s" bone="%s">' %
         (socketObject.name, self.name))
    func(' ' * depth + '  <transform>')
    func(' ' * depth + '    <vector x="%s" y="%s" z="%s" />' %
         (scale[0] * pos[0], scale[2] * pos[2], scale[1] * pos[1]))
    func(' ' * depth + '    <matrix>')
    if rot[0]:
